...
def alphacode(pin):
    ...
    mnemonic = ""
    while pin != 0:
        temp = pin % 100 # save the 2 low order digits of pin in temp
        pin = pin // 100 # remove the 2 low order digits from pin
        mnemonic = CONSONANTS[temp // 5] + VOWELS[temp % 5] + mnemonic # concatenate together
    return mnemonic
